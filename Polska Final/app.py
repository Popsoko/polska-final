...
def get_db_connection():
    ...
    # Add default events upon connection
    cursor = conn.cursor()
    default_events = [
        ("Student Camp", "Warsaw", datetime.utcnow(), 1),  # Assuming university_id 1 is for default events
        ("International Festival", "Lublin", datetime.utcnow(), 1),
        ("Uni Dance", "Gdansk", datetime.utcnow(), 2)   # Assuming university_id 2 is for another university
    ]
    cursor.executemany("INSERT INTO events (name, city, date, university_id) VALUES (?, ?, ?, ?)", default_events)
    conn.commit()
    cursor.close()
